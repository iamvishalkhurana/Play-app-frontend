...
import { zoomIn } from "../components/Utils/motion";
...
function AdminDashboard() {
  ...
  useEffect(() => {
    dispatch(getChannelStats({ userId }));
  }, [dispatch]);
  ...
  const fadeInVariants = () => {
    return {
      hidden: { opacity: 0 },
      show: { opacity: 1, duration: 2 },
    };
  };

  return (
    <motion.div variants={fadeInVariants()} animate="show" initial="hidden">
      <Navbar />
      <Container>
        <div className=" w-full relative h-screen text-white space-y-5 z-10 py-4 px-1">
          ...
          <StatsSection dashboard={dashboard} userId={userId} />
